...
def start():
    global game_state
    global BLACK
    start_page = pg.image.load(resource_path('cover_page_normal.png'))
    start_loc = (0, 0)
...
def prepare():
    ...
    background = pg.image.load(resource_path('background.png'))
    background_loc = (0, -1)
    cardback = pg.image.load(resource_path('cardback.png'))
    textsize = 25
    textfont = 'arial'
    ...
    for slot, card in list(player.slot.items()):
        if card is None:
            continue
        image = pg.image.load(resource_path(card + ".png"))
        image_loc = slot
    ...
    update_state()
    now = pg.image.load(resource_path(current_card + ".png"))
    now_loc = (529, 243)
...
def draw_card_animate(computer, times):
    ...
    while i <= times:
        i += 1
        image = pg.image.load(resource_path("cardback.png"))
        image_loc = (815, 450)
        if computer == "c1":
            card_loc = (55, 230)
            game_display.blit(image, card_loc)
...
def select_colour():
    white = pg.image.load(resource_path("white.png"))
...
def play_or_not():
    small_white = pg.image.load(resource_path("small_white.png"))
    ...
    game_display.blit(ask_txt_surf_obj, ask_txt_rect)
    game_display.blit(ans_txt_surf_obj, ans_txt_rect)
    just_drew = pg.image.load(resource_path(player.drew_card + ".png"))
...
# The sound effect
play_effect = pg.mixer.Sound((resource_path("effect.mp3")))
win_effect = pg.mixer.Sound((resource_path("win.mp3")))
lose_effect = pg.mixer.Sound((resource_path("lose.mp3")))
...
icon = pg.image.load(resource_path("unoicon.png"))
...
while True:
    ...
    if game_state == 2:
        ...
        if win_or_lose() == "win":
            win_effect.play()
            win_message = pg.image.load(resource_path("win.png"))
            game_display.blit(win_message, message_loc)
        if win_or_lose() == "lose":
            lose_effect.play()
            lose_message = pg.image.load(resource_path("lose.png"))
